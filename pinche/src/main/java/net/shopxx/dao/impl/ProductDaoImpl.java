...
public class ProductDaoImpl extends BaseDaoImpl<Product, Long> implements ProductDao {
	...
	public Page<Product> findPage(Product.Type type, Store store, ProductCategory productCategory, StoreProductCategory storeProductCategory, Brand brand, Promotion promotion, ProductTag productTag, StoreProductTag storeProductTag, Map<Attribute, String> attributeValueMap, BigDecimal startPrice,
		...
		if (orderType != null) {
			...
			case topDesc:
				...
				break;
			case priceAsc:
				criteriaQuery.orderBy(criteriaBuilder.asc(root.get("price")),criteriaBuilder.asc(root.get("marketPrice")),criteriaBuilder.desc(root.get("createdDate")));
				break;
			case priceDesc:
				criteriaQuery.orderBy(criteriaBuilder.desc(root.get("price")),criteriaBuilder.desc(root.get("marketPrice")), criteriaBuilder.desc(root.get("createdDate")));
				break;
			case salesDesc:
				criteriaQuery.orderBy(criteriaBuilder.desc(root.get("sales")), criteriaBuilder.desc(root.get("createdDate")));
				break;
			case salesAsc:
					criteriaQuery.orderBy(criteriaBuilder.asc(root.get("sales")), criteriaBuilder.desc(root.get("createdDate")));
					break;
			case scoreDesc:
				criteriaQuery.orderBy(criteriaBuilder.desc(root.get("score")), criteriaBuilder.desc(root.get("createdDate")));
				break;
			case dateDesc:
				...
				break;
				case monthHits:
					criteriaQuery.orderBy(criteriaBuilder.desc(root.get("monthHits")), criteriaBuilder.desc(root.get("createdDate")));
					break;
			}
	...
	@Override
	public List<Map<String, Object>> findProductByName(Store currentStore, String productName, int pageSize) {
		String sql="select id,name from Product where store_id = "+currentStore.getId()+" and name like '%"+productName+"%' order by id desc limit 0,"+pageSize;
	...
	@Override
	public List<Product> findGiftBag() {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Product> criteriaQuery = criteriaBuilder.createQuery(Product.class);
		Root<Product> root = criteriaQuery.from(Product.class);
		criteriaQuery.select(root);
		Predicate restrictions = criteriaBuilder.conjunction();
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("gradeDistributionFlag"), true));
		criteriaQuery.where(restrictions);
