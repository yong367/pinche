...
import net.shopxx.exception.ExcelException;
...
public class ExcelUtil {
    ...
    public static List<Map<String,String>> excelToList(InputStream in, String sheetName, String[] fields) throws ExcelException {
        //定义要返回的list
        ...
        try {
            ...
            for (int i = 1; i < realRows; i++) {
                ...
                for (int j=0;j<fields.length;j++) {
                    //获取当前单元格中的内容
                    String content = sheet.getCell(j, i).getContents().toString().trim();
                    row.put(fields[j], content);
                }
                resultList.add(row);
            }
        } catch (Exception e) {
