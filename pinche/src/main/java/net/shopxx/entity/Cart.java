...
public class Cart extends BaseEntity<Long> implements Iterable<CartItem> {
	 ...
	 * 所有商品结算标识
	 */
	private boolean settlementAllFlag=true;

	public boolean getSettlementAllFlag() {
		return settlementAllFlag;
	}

	public void setSettlementAllFlag(boolean settlementAllFlag) {
		this.settlementAllFlag = settlementAllFlag;
	}
	 ...
	 */
	@Transient
	public int getProductQuantity(Sku sku,int quantity) {
		Long productId = sku.getProduct().getId();
		int productQuantity = 0;
		if (getCartItems() != null) {
			for (CartItem cartItem : getCartItems(sku.getStore())) {
				if (productId.equals(cartItem.getSku().getProduct().getId())) {
	 ...
	 */
	@Transient
	public BigDecimal getRewardDuiban(Store store) {
		BigDecimal rewardDuiBan = BigDecimal.ZERO;
		if (getCartItems(store) != null) {
			for (CartItem cartItem : getCartItems(store)) {
				rewardDuiBan = rewardDuiBan.add(cartItem.getRewardDuiBan());
			}
		}
		return rewardDuiBan;
	}
	 ...
	 */
	@Transient
	public BigDecimal getRewardYongJin(Store store) {
		BigDecimal rewardYongJin = BigDecimal.ZERO;
		if (getCartItems(store) != null) {
			for (CartItem cartItem : getCartItems(store)) {
				rewardYongJin = rewardYongJin.add(cartItem.getRewardYongJin());
			}
		}
		return rewardYongJin;
	}
	 ...
	 */
	@Transient
	public BigDecimal getEffectiveRewardDuiBan(Store store) {
		BigDecimal effectiveRewardDuiBan = getRewardDuiban(store);
		return effectiveRewardDuiBan.compareTo(BigDecimal.ZERO) >= 0 ? effectiveRewardDuiBan : BigDecimal.ZERO;
	}
	 ...
	 */
	@Transient
	public BigDecimal getEffectiveRewardYongJin(Store store) {
		BigDecimal effectiveRewardYongJin = getRewardYongJin(store);
		return effectiveRewardYongJin.compareTo(BigDecimal.ZERO) >= 0 ? effectiveRewardYongJin : BigDecimal.ZERO;
	}
	...
	@Transient
	public List<CartItem> getLimitBuyProduct(){
		List<CartItem> result = new ArrayList<>();
		for (CartItem cartItem:getCartItems()) {
			Product pro=cartItem.getSku().getProduct();
			if(pro.getIsLimitBuy()){
				result.add(cartItem);
;			}
		}
		return result;
