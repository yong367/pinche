...
public class AlipayDirectPaymentPlugin extends PaymentPlugin {
    ...
    public void payHandle(PaymentPlugin paymentPlugin, PaymentTransaction paymentTransaction, String paymentDescription, String extra, HttpServletRequest request, HttpServletResponse response, ModelAndView modelAndView) throws Exception {
        ...
        alipayRequest.setBizContent("{\"out_trade_no\":\""+ paymentTransaction.getSn()+"\", \"total_amount\":" + paymentTransaction.getAmount().setScale(2).toString() + ",\"subject\":\""+StringUtils.abbreviate(paymentDescription.replaceAll("[^0-9a-zA-Z\\u4e00-\\u9fa5 ]", ""), 60)+"\",\"product_code\":\"QUICK_WAP_WAY\"}");//填充业务参数
        String form = alipayClient.pageExecute(alipayRequest,"get").getBody(); //调用SDK生成表单
       
        parameterMap.put("url",form);

        modelAndView.addObject("parameterMap", parameterMap);

        modelAndView.setViewName("/mobile/shop/payment/alipay_prepay");
    ...
    public QueryPayStatusResponse queryPaySuccess(PaymentTransaction paymentTransaction) throws Exception {
        ...
        request.setBizContent("{\"out_trade_no\":\""+ paymentTransaction.getSn()+"\"}");//填充业务参数
        ...
        if(response.isSuccess()){
            if("TRADE_SUCCESS".equals(response.getTradeStatus())){
                ...
                System.out.println("alipayDirectPaymentPlugin query  payResult is success,paymentTransactionSn:"+paymentTransaction.getSn());
            }else{
                queryPayStatusResponse.setPayStatus(false);
            }
        } else {
            queryPayStatusResponse.setPayStatus(false);
            queryPayStatusResponse.setMessage(response.getSubMsg());
            System.out.println("alipayDirectPaymentPlugin query  payResult is error,msg:"+queryPayStatusResponse.getMessage()+",paymentTransactionSn:"+paymentTransaction.getSn());
        }
