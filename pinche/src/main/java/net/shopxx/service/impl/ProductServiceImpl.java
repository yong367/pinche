...
public class ProductServiceImpl extends BaseServiceImpl<Product, Long> implements ProductService {
	...
	public Product create(Product product, Sku sku) {
		...
		if (sku.getRewardDuiBan() == null) {
			sku.setRewardDuiBan(BigDecimal.ZERO);
		}
		if (sku.getRewardYongJin() == null) {
			sku.setRewardYongJin(BigDecimal.ZERO);
		}
	...
	public Product modify(Product product, Sku sku) {
		...
		if (pProduct.hasSpecification()) {
			...
			defaultSku.setRewardDuiBan(sku.getRewardDuiBan());
			defaultSku.setRewardYongJin(sku.getRewardYongJin());
		...
		if (pProduct.hasSpecification()) {
			...
			for (Sku sku : skus) {
				...
				if (pSku != null) {
					...
					pSku.setRewardDuiBan(sku.getRewardDuiBan());
					pSku.setRewardYongJin(sku.getRewardYongJin());
	...
	public List<Map<String, Object>> findProductByName(Store currentStore, String productName, int pageSize) {
		return productDao.findProductByName(currentStore,productName,pageSize);
	}

	@Override
	@Transactional(readOnly = true)
	@Cacheable(value = "productRiftBag", condition = "#useCache")
	public List<Product> findGiftBag(boolean useCache) {
		return productDao.findGiftBag();
	}
}