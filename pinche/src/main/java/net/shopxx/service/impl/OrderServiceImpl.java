...
import net.shopxx.component.ProductCouponCahceManage;
import net.shopxx.component.WeiXinManage;
import net.shopxx.dao.*;
import net.shopxx.entity.*;
import net.shopxx.service.*;
import net.shopxx.util.SystemUtils;
import net.shopxx.weixin.template.message.TicketNotifyMessage;
...
public class OrderServiceImpl extends BaseServiceImpl<Order, Long> implements OrderService {
	...
	@Inject
	private ProductCouponCahceManage productCouponCahceManage;
	@Inject
	...
	public List<Order> generate(Order.Type type, Cart cart, Receiver receiver, PaymentMethod paymentMethod, ShippingMethod shippingMethod, CouponCode couponCode, Invoice invoice, BigDecimal balance, String memo) {
		...
		for (Store store : stores) {
			...
			order.setRewardDuiBan(cart.getEffectiveRewardDuiBan(store));
	...
	public List<Order> create(Order.Type type, Cart cart, Receiver receiver, PaymentMethod paymentMethod, ShippingMethod shippingMethod, CouponCode couponCode, Invoice invoice, BigDecimal balance, String memo) {
		...
		for (Store store : stores) {
			...
			order.setRewardDuiBan(cart.getEffectiveRewardDuiBan(store));
			order.setRewardYongJin(cart.getEffectiveRewardYongJin(store));
			order.setExchangePoint(cart.getExchangePoint(store));
			order.setWeight(cart.getTotalWeight(store));
			order.setQuantity(cart.getTotalQuantity(store));
			order.setShippedQuantity(0);
			...
			if (amountPayable.compareTo(BigDecimal.ZERO) > 0) {
				...
				order.setStatus(PaymentMethod.Type.deliveryAgainstPayment.equals(paymentMethod.getType()) ? Order.Status.pendingPayment : Order.Status.pendingShipment);
				order.setPaymentMethod(paymentMethod);
...
//			businessService.addBalance(order.getStore().getBusiness(), order.getRefundableAmount(), BusinessDepositLog.Type.orderRefunds, null);
			memberService.addBalance(order.getMember(), order.getRefundableAmount(), MemberDepositLog.Type.orderRefunds, null);
			order.setAmountPaid(BigDecimal.ZERO);
		}
	...
	@Override
	public void sendProductCoupon(Order order) {
		for (OrderItem orderItem:order.getOrderItems()) {
			Sku sku=orderItem.getSku();
			Product product = sku.getProduct();
			if(product.getType().equals(Product.Type.coupon)){
//				List<ProductCoupon> productCoupons=productCouponCahceManage.getProductCoupon(String.valueOf(sku.getId()),orderItem.getQuantity());
				List<ProductCoupon> result=productCouponService.findListByOrderSn(order.getSn());
				if(result ==null || result.size()==0){
					for (int i = 0; i < orderItem.getQuantity(); i++) {
							int count=0;
							ProductCoupon productCoupon =null;
							while (count==0){
								productCoupon=productCouponCahceManage.getProductCoupon(String.valueOf(sku.getId()));
								count=productCouponService.updateStatus(productCoupon,order.getSn());
								if(count>0){
									wxNotifyMessage(productCoupon.getCodeValue(),productCoupon.getSoftName(),order.getMember());
								}
							}
							logPrintService.printServerLog("orderNo："+order.getSn()+",allot productCoupon ="+productCoupon.getCodeValue()+",success");
					}
				}
			}
	...
	public void wxNotifyMessage(String ticketCode,String softName, Member currentUser) {
		if (currentUser.getSocialUsers().size() > 0 ) {
			...
			while (it.hasNext()){
				...
				if(socialUser.getLoginPluginId().equals("weixinLoginPlugin")){
					...
					TicketNotifyMessage applyCashNotifyMessage = new TicketNotifyMessage();
					applyCashNotifyMessage.setTicketCode(ticketCode);
					applyCashNotifyMessage.setSoftName(softName);
	...
	@Override
	public void qrOrdercomplete(Order order) {
		complete(order);
		for (OrderItem orderItem : order.getOrderItems()) {
			Sku sku = orderItem.getSku();
			if (sku != null && sku.getProduct() != null) {
				skuService.addStock(sku, -orderItem.getQuantity(), StockLog.Type.stockOut, null);
			}
			if (BooleanUtils.isTrue(order.getIsAllocatedStock())) {
				skuService.addAllocatedStock(sku, -orderItem.getQuantity());
			}
		}
	}
	...
	public void complete(Order order) {
		...
		Business business = order.getStore().getBusiness();
		if (order.getRewardPoint() > 0) {
			memberService.addPoint(member, order.getRewardPoint(), PointLog.Type.merchantReward, order.getStore().getName()+"消费赠送");
		}
		...
		if (order.getSettlementAmount().compareTo(BigDecimal.ZERO) > 0) {
			businessService.addBalance(business, order.getSettlementAmount(), BusinessDepositLog.Type.orderSettlement, null);
		}
		for (OrderItem orderItem : order.getOrderItems()) {
			Sku sku = orderItem.getSku();
			if (sku != null && sku.getProduct() != null) {
		...
		orderLog.setOrder(order);
		orderLogDao.persist(orderLog);
		processPromotionReward(order);//处理推广奖励
	}



	private void processPromotionReward(Order order){
		BigDecimal rewardDuiBan = order.getRewardDuiBan();
		BigDecimal rewardYongJin = order.getRewardYongJin();
		Member member = order.getMember();
		Business business=order.getStore().getBusiness();

		//设置赠送兑伴比例
		if(rewardDuiBan !=null && rewardDuiBan.compareTo(BigDecimal.ZERO)>0){
			memberService.addBalance(member,rewardDuiBan,MemberDepositLog.Type.promotionReward,"推广奖励");
			businessService.addBalance(business,rewardDuiBan.negate(), BusinessDepositLog.Type.promotionReward, "推广奖励");
		}
		//设置邀请人返佣比例，不设置赠送兑伴比例
		if (rewardYongJin != null && rewardYongJin.compareTo(BigDecimal.ZERO) > 0 ) {
			//memberService.addBalance(member,rewardYongJin,MemberDepositLog.Type.promotionReward,"推广奖励");
			Member recommendMember = memberService.find("shareCode",member.getRecommendCode());
			if(recommendMember != null){
				//BigDecimal memberParentDonationAmount = memberParentdonationRatio.multiply(rewardYongJin);
				//reward(member, business, recommendMember, rewardYongJin);
				memberService.addBalance(recommendMember,rewardYongJin, MemberDepositLog.Type.promotionReward,"推广奖励");
				businessService.addBalance(business,rewardYongJin.negate(), BusinessDepositLog.Type.promotionReward, "推广奖励");
				memberContributionService.addContributionLog(member,recommendMember, MemberContributionLog.Type.order,rewardYongJin);//保存贡献值记录
			}
		}
	}
	...
	private void exchangePoint(Order order) {
		...
		memberService.addPoint(order.getMember(), -order.getExchangePoint(), PointLog.Type.exchange, "兑换店铺【"+order.getStore().getName()+"】商品,商品名称【"+order.getOrderItems().get(0).getName()+"】");
	...
	private void undoExchangePoint(Order order) {
		...
		memberService.addPoint(order.getMember(), order.getExchangePoint(), PointLog.Type.undoExchange, "兑换"+order.getStore().getName()+"店商品撤销");
		order.setIsExchangePoint(false);
	}
