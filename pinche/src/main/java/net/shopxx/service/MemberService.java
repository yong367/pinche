...
public interface MemberService extends BaseService<Member, Long>, AuthenticationProvider {
	...
	/**
	 * 赠送转增积分
	 * @param currentUser 送出积分用户
	 * @param riceiveMember 接受积分用户
	 * @param pointNum 积分数量
	 */
    void presentationPoint(Member currentUser, Member riceiveMember, long pointNum);

    void updateCreateTime(Member member);
	 ...
	 * @return
	 */
	Member findByRecommendCode(String recommendCode);

    void exchangeMemberLevel(Member currentUser, MemberRank newMemberRank);

    void takePrizeDeduct(Member currentUser);
}