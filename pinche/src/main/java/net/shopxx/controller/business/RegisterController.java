...
public class RegisterController extends BaseController {
	...
	public ResponseEntity<?> submit(String username, String password, String email, String mobile, HttpServletRequest request) {
		...
		userService.login(new UserAuthenticationToken(Business.class, username, business.getEncodedPassword(), false, request.getRemoteAddr()));
	 ...
	 */
	@PostMapping("/submit")
	public ResponseEntity<?> submit( String password,String mobile,String validateNo, HttpServletRequest request) {
		Setting setting = SystemUtils.getSetting();
		if (!ArrayUtils.contains(setting.getAllowedRegisterTypes(), Setting.RegisterType.business)) {
			return Results.unprocessableEntity("business.register.disabled");
		}
		if ( !isValid(Business.class, "password", password, BaseEntity.Save.class)  || !isValid(Business.class, "mobile", mobile, BaseEntity.Save.class)) {
			return Results.UNPROCESSABLE_ENTITY;
		}
