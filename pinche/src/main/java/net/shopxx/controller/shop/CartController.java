...
public class CartController extends BaseController {
	...
	public ResponseEntity<?> add(Long skuId, Integer quantity, @CurrentCart Cart currentCart) {
		...
		if (!Product.Type.general.equals(sku.getType()) && !Product.Type.coupon.equals(sku.getType())) {
		...
		if(product.getIsLimitBuy() && currentCart != null){
			if(currentCart.getProductQuantity(sku,quantity) > product.getLimitBuyCount()){
				return  Results.customEntity("该商品为限制购买商品，不能超过商品的限制的购买数量！");
			}
		}
		if (currentCart == null) {
		...
		return Results.ok("shop.cart.addSuccess", currentCart.getSkuQuantity(null), currency(currentCart.getEffectivePrice(null), false, false));
	}
	...
	public Object size() {
		...
		if(cart!=null){
			for (CartItem cartItem:cart.getCartItems()) {
				...
				bigAmount = bigAmount.add(cartItem.getSubtotal());
			}
		}
		result.put("size",size);
		result.put("amount",currency(bigAmount,false,false));
		return result;
	}
	...
	public ResponseEntity<?> modify(Long skuId, Integer quantity, @CurrentCart Cart currentCart) {
		...
		Product product = sku.getProduct();
		if(product.getIsLimitBuy()){
			if(quantity > product.getLimitBuyCount()){
				return  Results.customEntity("该商品为限制购买商品，不能超过商品的限制的购买数量！");
			}
		}
