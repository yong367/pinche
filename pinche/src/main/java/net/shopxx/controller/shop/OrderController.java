...
public class OrderController extends BaseController {
	 ...
	 */
	@GetMapping("/checkout")
	public String checkout(@CurrentUser Member currentUser, @CurrentCart Cart currentCart, ModelMap model, RedirectAttributes redirectAttributes,@RequestParam(name = "allProduct",defaultValue = "true")Boolean allProduct
			, @RequestParam(name = "skuids",defaultValue = "")String skuIds) {
		currentCart.setSettlementAllFlag(allProduct);
		cartService.update(currentCart);
		Cart newCart =currentCart;
		if(!allProduct){
			newCart =new Cart();
			newCart.setMember(currentUser);
			cartItemService.updateCartItemSettlementFlag(currentCart.getId());
			String[]ids=skuIds.split(",");
			Set<CartItem> set=currentCart.getCartItems();
			Iterator<CartItem> it=set.iterator();
			for (String id:ids) {
				while (it.hasNext()){
					CartItem cartItem=it.next();
					if(cartItem.getSku().getId().toString().equals(id)){
						cartItem.setSettlementFlag(true);
						cartItemService.updateCartItemFlag(cartItem);
		...
		if(StringUtils.isEmpty(skuIds)){
			addFlashMessage(redirectAttributes, "结算商品不能为空！");
			return "redirect:/cart/list";
		}
		if (newCart == null || newCart.isEmpty()) {
			return "redirect:/cart/list";
		}
		if (newCart.hasNotActive(null)) {
			addFlashMessage(redirectAttributes, "shop.order.hasNotActive");
			return "redirect:/cart/list";
		}
		if (newCart.hasNotMarketable(null)) {
			addFlashMessage(redirectAttributes, "shop.order.hasNotMarketable");
			return "redirect:/cart/list";
		}
		List<CartItem> limitProduct = newCart.getLimitBuyProduct();
		if(limitProduct.size()>0){ //判断用户的购物车中是否有限购物品
			String showMessage="";
			boolean showFlag =false;
			for (CartItem cartItem:limitProduct) {
				Product product = cartItem.getSku().getProduct();
				int productBuyCount = newCart.getProductQuantity(cartItem.getSku(), 0);
				int count =orderItemService.limitProductBuyCount(product,currentUser);
				if(count >= product.getLimitBuyCount() || (productBuyCount+count) > product.getLimitBuyCount()){
					showMessage += product.getName()+",";
					showFlag = true;
				}
			}
			if(showFlag){
				showMessage +="商品超过限制购买数量！";
				addFlashMessage(redirectAttributes, showMessage);
				return "redirect:/cart/list";
			}
		...
		List<Order> orders = orderService.generate(Order.Type.general, newCart, defaultReceiver, null, null, null, null, null, null);
	...
	public ResponseEntity<?> create(String cartTag, Long receiverId, Long paymentMethodId, Long shippingMethodId, String code, String invoiceTitle, BigDecimal balance, String memo, @CurrentUser Member currentUser, @CurrentCart Cart currentCart) {
		Cart newCart = currentCart;
		if(!currentCart.getSettlementAllFlag()){
			//删除原购物车中的已结算的item
			newCart=new Cart();
			newCart.setMember(currentCart.getMember());
			Set<CartItem> set=currentCart.getCartItems();
			Iterator<CartItem> it=set.iterator();
			while (it.hasNext()){
				CartItem cartItem=it.next();
				if(cartItem.getSettlementFlag()){
					newCart.add(cartItem);
				}
			}
		}
		...
		if (!StringUtils.equals(newCart.getTag(), cartTag)) {
		...
		CouponCode couponCode = couponCodeService.findByCode(code);
		if (couponCode != null && couponCode.getCoupon() != null && !newCart.isValid(couponCode, couponCode.getCoupon().getStore())) {
			return Results.UNPROCESSABLE_ENTITY;
		}
		if (balance != null && balance.compareTo(BigDecimal.ZERO) < 0) {
			return Results.UNPROCESSABLE_ENTITY;
		...
		Invoice invoice = StringUtils.isNotEmpty(invoiceTitle) ? new Invoice(invoiceTitle, null) : null;
		List<Order> orders = orderService.create(Order.Type.general, newCart, receiver, paymentMethod, shippingMethod, couponCode, invoice, balance, memo);
		...
		if(currentCart.getSettlementAllFlag()){
			if (!currentCart.isNew()) {
				cartService.remove(currentCart);
			}
		}else{
			Set<CartItem> set=currentCart.getCartItems();
			Iterator<CartItem> it=set.iterator();
			while (it.hasNext()){
				CartItem cartItem=it.next();
				if(cartItem.getSettlementFlag()){
					cartService.remove(currentCart,cartItem.getSku());
				}
			}
		}
