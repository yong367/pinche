...
public class ProductController extends BaseController {
	...
	public String detail(@PathVariable Long productId, ModelMap model, HttpServletRequest request) {
		...
		try {
			...
			logPrintService.printServerLog("微信获取相关认证信息出错"+e.getMessage());
		}
		...
		model.addAttribute("productImg", product.getProductImages().size() > 0 ? product.getProductImages().get(0).getSource():product.getImage());
		if(product.getProductImages().size() > 0){
			model.addAttribute("productImg", product.getProductImages().get(0).getSource());
		} else {
			model.addAttribute("productImg", "http://www.lifeabb.com/upload/image/shareProductList.jpg");
		}
	 ...
	 * 列表
	 */
	@GetMapping("/listByProductTag/{productTagId}")
	public String list(@PathVariable Long productTagId, Product.Type type, Long brandId, Long promotionId, BigDecimal startPrice, BigDecimal endPrice, Product.OrderType orderType, Integer pageNumber, Integer pageSize, HttpServletRequest request, ModelMap model) {
		ProductTag  productTag= productTagService.find(productTagId);
		if (productTag == null) {
			throw new ResourceNotFoundException();
		}
		...
		model.addAttribute("pageNumber", pageNumber);
		model.addAttribute("pageSize", pageSize);
		model.addAttribute("page", productService.findPage(type, null, null, null, brand, promotion, productTag, null, null, startPrice, endPrice, true, true, null, true, null, null, null, orderType, pageable));
		return "shop/product/list";
	}
