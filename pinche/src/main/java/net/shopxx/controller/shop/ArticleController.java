...
public class ArticleController extends BaseController {
	 ...
	 * 列表
	 */
	@GetMapping("/notify/list/{articleCategoryId}")
	public ResponseEntity<?> notifyList(@PathVariable Long articleCategoryId, Integer pageNumber, ModelMap model) {
		ArticleCategory articleCategory = articleCategoryService.find(articleCategoryId);
		if (articleCategory == null) {
			throw new ResourceNotFoundException();
		}
        ...
        data.put("content",articleService.findPage(articleCategory, null, true, pageable).getContent());
        return ResponseEntity.ok(data);
	}
}