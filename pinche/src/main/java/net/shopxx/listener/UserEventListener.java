...
public  class UserEventListener {
	...
	public void handle(UserRegisteredEvent userRegisteredEvent) {
		...
		if (user instanceof Member) {
			...
			if (setting.getRegisterPoint() > 0) {
				memberService.addPoint(member, setting.getRegisterPoint(), PointLog.Type.systemReward, "帐号注册奖励");
				if(StringUtils.isNotEmpty(member.getRecommendCode())){
					Member parentMember = memberService.findByRecommendCode(member.getRecommendCode());
					if(parentMember!=null && setting.getInviteRegisterPoint()>0){
						memberService.addPoint(parentMember, setting.getInviteRegisterPoint(), PointLog.Type.systemReward, "邀请用户注册奖励");
					}
				}

			}
			generateShareInfo(member);
	 ...
	 * @param member
	 */
	public void generateShareInfo(Member member){
		if(StringUtils.isEmpty(member.getShareCode())|| StringUtils.isEmpty(member.getShareCodeImgUrl())) {
            if (StringUtils.isEmpty(member.getShareCode())) {
                member.setShareCode(shareCodeService.generateShareCode());
            }
            if (StringUtils.isEmpty(member.getShareCodeImgUrl())) {
                member.setShareCodeImgUrl(shareCodeService.generateShareCodeImgUrl(member));
            }
