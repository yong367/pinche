<div class="span2">
	<div class="memberInfo">
		...
		<p>
			...
			<span class="red"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(currentUser.balance, false, true)}</span>
		</p>
		<p>
			${message("member.index.amount")}:
			<span class="red"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(currentUser.amount, false, true)}</span>
		</p>
		<p>
	...
	<div class="menu">
		...
		<dl>
			...
			<dd [#if current == "depositRecharge"] class="current"[/#if]>
				<a href="${base}/member/deposit/recharge">兑伴儿充值</a>
			</dd>
			<dd class="last[#if current == "memberDepositLog"] current[/#if]">
				<a href="${base}/member/deposit/log">我的兑伴儿</a>
			</dd>
		</dl>
	</div>
</div>