										...
										<div class="row">
											<div class="col-xs-6">
												<dl class="items dl-horizontal clearfix">
													...
													<dd><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${message("business.store.serviceFee", currency(currentStore.storeRank.serviceFee, false))}</dd>
													[#if currentStore.status == "approved"]
														<dt>${message("StoreCategory.bail")}:</dt>
														<dd><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(currentStore.bailPayable, false)}</dd>
													[/#if]
												</dl>
											</div>
										</div>
										...
										<div class="row">
											<div class="col-xs-4 col-xs-offset-2">
												[#if currentStore.endDate?? || currentStore.status != "success"]
													<button class="btn btn-primary" type="button" onclick="payment()" id="paymentButton">${message("business.common.submit")}</button>
												[/#if]
