...
function showFenLei(){
        ...
        function initData(ordername,upOrDown){
            if(ordername =='monthHits'){
                window.location.href='${base}/store/allProduct/hot_down/${store.id}/0';
            }
            if(ordername =='sales'){
                if(upOrDown =="up"){
                    window.location.href='${base}/store/allProduct/sales_up/'+${store.id}+'/'+fenLeiId;
                }else{
                    window.location.href='${base}/store/allProduct/sales_down/'+${store.id}+'/'+fenLeiId;
                }
            }
            if(ordername =='price'){
                window.location.href='${base}/store/allProduct/price_up/${store.id}/0';
            }
            /*$("#shopList").html("");
            var $shopList = $("#shopList");
            var shopListTemplate = _.template($("#shopListTemplate").html());
        ...
        function selectFenLei(id) {
            ...
            if(orderName=='priceAsc'){
                window.location.href='${base}/store/allProduct/sales_up/'+${store.id}+'/'+id;
            }else{
                window.location.href='${base}/store/allProduct/sales_down/'+${store.id}+'/'+id;
            }
        }
        function submitForm() {
...
<div class="home_head orange">
    <form class="flex flex-align-center home_head_form" action="/store/search" method="post" id="searchForm">
        ...
        <a href="/" class="head_back">
            <div class="flex flex-align-center head_back_div"><button class="icon_1 icon_1_back"></button></div>
        </a>
...
<div class="flex flex-align-center shop_list_type">
    ...
    <div class="flex-1 shop_list_type_item">
        <input type="radio" name="shop_list_type" lt="sales" id="slt2" onclick="shop_list_type_change(this)" checked [#if orderName=="salesAsc"]up="1"[/#if]>
    ...
    <div class="flex-1 shop_list_type_item">
        <input type="radio" name="shop_list_type" lt="price" id="slt3" onclick="shop_list_type_change(this)">
