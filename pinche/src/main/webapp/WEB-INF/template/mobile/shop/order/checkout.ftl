...
<head>
	...
	<script type="text/javascript">
					...
					success: function(data) {
						$dataItem.each(function() {
							...
							if (name in data) {
								...
								switch(type) {
									case "currency":
										$element.html(currency(value, false, true));/*'<img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>'+*/
										break;
									default:
										$element.text(value);
								}
		...
		function goBack(){
		    var firstStoreid= $($("input[class='storeId']")[0]).val();
		    window.location.href="/store/"+firstStoreid;
		}

        /**
		 * 如果配送和支付方式就一个的话 就默认选中
         */
		$(function () {
            initPaymentMethod();
            initShippingMethod();
		...
		function initPaymentMethod(){
		    var list =$("#paymentMethodPage").find(".list-group-item");
		    if(list.length==1){
		        $(list[0]).trigger("click");
		        /*$("#paymentMethodId").val($(list[0]).attr("data-payment-method-id"));
                $("#defaultPaymentMethodName").html($(list[0]).attr("data-payment-method-name"));
                $("#defaultPaymentMethodNameSpan").hide();*/
                $("#defaultPaymentMethodNameSpan").hide();
			}
		}
        function initShippingMethod(){
            var list =$("#shippingMethodPage").find(".list-group-item");
            if(list.length==1){
                $(list[0]).trigger("click");
               /* $("#shippingMethodId").val($(list[0]).attr("data-shipping-method-id"));
                $("#defaultShippingMethodName").html($(list[0]).attr("data-shipping-method-name"));*/
                $("#defaultShippingMethodNameSpan").hide();
            }
        }
	</script>
</head>
<body class="order-checkout">
	<div id="mainPage" class="main-page page">
		<form id="orderForm" action="create" method="post">
			...
			<div class="header-fixed" style="background:linear-gradient(-85deg,rgba(169,207,252,1) 0%,rgba(144,164,251,1) 21%,rgba(148,168,254,1) 54%,rgba(148,168,254,1) 71%,rgba(213,190,218,1) 100%);color: #FFFFFF">
				<a class="pull-left" href="javascript:void(0)" onclick="goBack()">
				...
				<div class="container-fluid">
							...
							<div class="media">
								...
								<div class="media-body media-middle">
									[#if defaultReceiver??]
										...
										<span style="font-size: 16px;" class="small">${defaultReceiver.areaName}${defaultReceiver.address}</span>
                                       <div style="height: 10px;"></div>
										<span class="pull-left">${defaultReceiver.phone}</span>
									[#else]
					...
					<div class="list-group list-group-flat">
						<div id="currentPaymentMethod" class="[#if amountPayable <= 0]hidden-element [/#if]list-group-item text-right" data-toggle="page" data-target="#paymentMethodPage">
							...
							<span class="name gray-darker" id="defaultPaymentMethodName">${(defaultPaymentMethod.name)!message("shop.order.choose")}</span>
							<span class="glyphicon glyphicon-menu-right" id="defaultPaymentMethodNameSpan"></span>
						</div>
						[#if isDelivery]
							<div id="currentShippingMethod" class="list-group-item text-right" data-toggle="page" data-target="#shippingMethodPage">
								...
								<span class="name gray-darker" id="defaultShippingMethodName">${(defaultShippingMethod.name)!message("shop.order.choose")}</span>
								<span class="glyphicon glyphicon-menu-right" id="defaultShippingMethodNameSpan"></span>
							</div>
						[/#if]
						[#if orderType == "general"]
						...
						[#if currentUser.balance > 0]
							<div id="useBalanceItem" class="[#if amount <= 0]hidden-element [/#if]list-group-item">
								<div class="row">
									...
									<div class="col-xs-7">
										<span class="gray-darker">${message("shop.order.balance")}: <img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(currentUser.balance, false)}</span>
									</div>
									<div class="col-xs-2 text-right">
					...
					<div class="list-group list-group-flat">
						[#if orderType == "general"]
							...
							<div class="list-group-item small">
								...
								<strong class="pull-right" data-name="promotionDiscount" data-type="currency"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(promotionDiscount, false, true)}</strong>
							</div>
							<div class="list-group-item small">
								${message("Order.couponDiscount")}
								<strong class="pull-right" data-name="couponDiscount" data-type="currency"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(couponDiscount, false, true)}</strong>
							</div>
							[#if setting.isInvoiceEnabled && setting.isTaxPriceEnabled]
								<div class="list-group-item small">
									${message("Order.tax")}
									<strong class="pull-right" data-name="tax" data-type="currency"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(tax, false, true)}</strong>
								</div>
							[/#if]
						[#elseif orderType == "exchange"]
							<div class="list-group-item small">
						...
						[#if isDelivery]
							<div class="list-group-item small">
								...
								<strong class="pull-right" data-name="freight" data-type="currency"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(freight, false, true)}</strong>
							</div>
						[/#if]
					</div>
				</div>
			</div>
			<div class="footer-fixed" style="background: rgb(197,184,226);">
				<div class="container-fluid">
					<div class="row">
						<div class="col-xs-8 text-center" style="color: #FFFFFF">
							[#if orderType == "general"]
								共计:
								<strong class="red" data-name="amount" data-type="currency" style="color: #FFFFFF">[#--<img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>--]${currency(amount, false, true)}</strong>
							[#elseif orderType == "exchange"]
								共计:
								<strong class="red" style="color: #FFFFFF">${exchangePoint}</strong>
							[/#if]
						</div>
						<div class="col-xs-4">
							<button id="submit" class="btn btn-red btn-flat btn-block" type="button" style="background: rgb(148,168,254);">${message("shop.order.checkout")}</button>
						</div>
					</div>
				</div>
			</div>
	...
	<div id="receiverPage" class="receiver-page page">
		...
		<div class="footer-fixed" >
			<button id="addReceiver" class="btn btn-red btn-flat btn-block" style="background: rgb(148,168,254);" type="button" data-toggle="page" data-target="#addReceiverPage">${message("shop.order.addReceiver")}</button>
		</div>
	</div>
	<div id="addReceiverPage" class="add-receiver-page page">
		<form id="addReceiverForm" action="receiver_save" method="post">
			...
			<div class="footer-fixed" >
				<button class="btn btn-red btn-flat btn-block" type="submit" style="background: rgb(148,168,254);">${message("shop.order.useAndSave")}</button>
			</div>
		</form>
	</div>
	<div id="paymentMethodPage" class="payment-method-page page">
		...
		<div class="footer-fixed" >
			<button class="btn btn-red btn-flat btn-block" type="button" data-toggle="page" data-target="#mainPage" style="background: rgb(148,168,254);">${message("shop.order.close")}</button>
		</div>
	</div>
	<div id="shippingMethodPage" class="shipping-method-page page">
		...
		<div class="footer-fixed" >
			<button class="btn btn-red btn-flat btn-block" style="background: rgb(148,168,254);" type="button" data-toggle="page" data-target="#mainPage">${message("shop.order.close")}</button>
		</div>
	</div>
	[#--数据监控--]
    <script>
