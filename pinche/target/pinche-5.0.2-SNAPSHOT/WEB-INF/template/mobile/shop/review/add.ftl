...
<body class="add-review">
	...
	<main>
		<div class="media">
			...
			<div class="media-body">
				...
				<span>
					...
					<strong class="red"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(product.price, false, true)}</strong>
				</span>
				...
				[#elseif setting.isShowMarketPrice]
					<span>
						...
						<del class="gray-darker"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(product.marketPrice, false, true)}</del>
					</span>
				[/#if]
			</div>
		</div>
		<form id="reviewForm" action="${base}/review/save" method="post">
			...
			[/#if]
			<button class="btn btn-lg btn-primary btn-block" type="submit" style="background-color: #2cb6f1">${message("shop.review.submit")}</button>
		</form>
	</main>
</body>
</html>