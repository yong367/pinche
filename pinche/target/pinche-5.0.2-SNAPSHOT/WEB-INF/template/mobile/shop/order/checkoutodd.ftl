					...
					success: function(data) {
						$dataItem.each(function() {
							...
							if (name in data) {
								...
								switch(type) {
									case "currency":
										$element.html('<img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>'+currency(value, false, true));
										break;
									default:
										$element.text(value);
								}
					...
					<div class="row">
						<div class="col-xs-8 text-center">
							[#if orderType == "general"]
								...
								<strong class="red" data-name="amount" data-type="currency"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(amount, false, true)}</strong>
							[#elseif orderType == "exchange"]
								${message("Order.exchangePoint")}:
