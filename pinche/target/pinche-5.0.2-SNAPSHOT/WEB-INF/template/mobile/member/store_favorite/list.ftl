		...
		<%_.each(stores, function(storeFavorite, i) {%>
			<div class="panel panel-flat">
				...
				<div class="panel-footer text-right">
					...
					<a style="border:none" class="delete btn btn-sm btn-default" href="javascript:;" data-store-favorite-id="<%-storeFavorite.id%>">${message("member.common.delete")}</a>
					<a style="border-color:#15b1ee;color:#15b1ee;" class="btn btn-sm btn-default" href="${base}<%-storeFavorite.store.path%>">${message("member.storeFavorite.inShop")}</a>
				</div>
			</div>
		<%})%>
	</script>
...
<body class="profile">
	<header class="header-fixed" style="background:linear-gradient(86deg,rgba(150,100,111,1) 0%,rgba(107,107,138,1) 50%,rgba(217,124,117,1) 100%);color: #FFFFFF">
