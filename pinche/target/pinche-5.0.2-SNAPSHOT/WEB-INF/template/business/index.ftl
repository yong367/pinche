								...
								<div class="row">
									...
									<div class="col-xs-6">
										<p>
											...
											<span class="orange"><img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(currentUser.balance, false, true)}</span>
											[#if currentUser.frozenFund > 0]
												<span class="gray-darker">(冻结兑伴儿: <img style="width: 15px;height: 15px;" src="${setting.currencySign}"  alt="${setting.currencyIconName}"/>${currency(currentUser.frozenFund, false)})</span>
											[/#if]
										</p>
										<p>
...
[#if currentStore?? && currentStore.status == "success"]
						<div class="row">
                            <div style="width: 100%;text-align: center;">
								<div id="qrCode" ></div>
								<div style="padding-top: 15px;font-size: 20px;font-weight: bold;font-family: "微软雅黑", "SimHei", "Microsoft YaHei", "Helvetica Neue", Helvetica, STHeiTi, sans-serif">用户扫码直接打开手机端商铺首页</div>
                            </div>
						</div>
[/#if]
				</section>
