		...
		$().ready(function() {
			...
			[#if product.hasSpecification()]
				[#list product.skus as sku]
					initSkuValues["${sku.specificationValueIds?join(",")}"] = {
                        ...
                        rewardDuiBan: ${sku.rewardDuiBan},
                        rewardYongJin: ${sku.rewardYongJin},
			...
			function changeView() {
				if (hasSpecification) {
					...
					$price.add($cost).add($marketPrice).add($rewardPoint).add($rewardDuiBan).add($rewardYongJin).add($exchangePoint).add($stock).prop("disabled", true).closest("div.form-group").velocity("slideUp");
				} else {
					$isDefault.prop("disabled", false);
					$price.add($cost).add($marketPrice).add($rewardPoint).add($rewardDuiBan).add($rewardYongJin).add($exchangePoint).add($stock).prop("disabled", false).closest("div.form-group").velocity("slideUp");
				}
			}
											...
											<div id="base" class="tab-pane active">
												...
												[#if product.type == "general" || product.type == "coupon"]
													<div class="form-group">
														...
														<div class="col-xs-4">
															<input id="price" name="sku.price" class="form-control" type="text" value="${product.defaultSku.price}" maxlength="16">
														</div>
													</div>
												[/#if]
												...
												[#if product.type == "general"]
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="col-xs-4">
                                                            <input id="rewardDuiBan" name="sku.rewardDuiBan" class="form-control" type="text" value="${product.defaultSku.rewardDuiBan}" maxlength="9" title="赠送兑伴">
                                                            [#--<label class="control-label" >该值为奖励购买者所赠送的积分数额，如该购买用户老用户推荐购买，则会额外赠送给推荐这该值10%数额的兑伴积分作为奖励</label>--]
														</div>

                                                    </div>
													 <div class="form-group">
														 <label class="col-xs-2 control-label" for="rewardDuiBan">赠送返佣:</label>
														 <div class="col-xs-4">
															 <input id="rewardYongJin" name="sku.rewardYongJin" class="form-control" type="text" value="${product.defaultSku.rewardYongJin}" maxlength="9" title="赠送返佣">
														 </div>
												...
												<div class="form-group">
													...
													<div class="col-xs-10 checkbox">
                                                        ...
                                                        <input id="isLimitBuy" name="isLimitBuy" type="text" value="${product.isLimitBuy?string("true", "false")}" data-toggle="checkbox-x" onchange="showLimitBuyCount(this)">
                                                        <label for="isLimitBuy" class="cbx-label">是否限购</label>
													</div>
												</div>
                                                <div class="form-group" id="limitBuyCountDiv" [#if !product.isLimitBuy ]style="display: none"[/#if]>
                                                    <label class="col-xs-2 control-label" for="limitBuyCount">限购次数:</label>
                                                    <div class="col-xs-4">
                                                        <input id="limitBuyCount" name="limitBuyCount" class="form-control" type="text" maxlength="200" value="${product.limitBuyCount}" [#if !product.isLimitBuy ]disabled="disabled"[/#if]>
                                                    </div>
                                                </div>
												<div class="form-group">
												...
												[#if gradeGiftBag]
													<div class="form-group">
														...
														<div class="col-xs-4">
															<input name="gradeDistributionFlag" type="radio" value="true" [#if product.gradeDistributionFlag] checked [/#if]>
															<label for="taskShowFlag" class="cbx-label">是</label>
															<input name="gradeDistributionFlag" type="radio" value="false" [#if !product.gradeDistributionFlag] checked [/#if]>
															<label for="taskShowFlag" class="cbx-label">否</label>
														</div>
													</div>
													<div class="form-group" id="memberrankDiv" style="[#if product.gradeDistributionFlag] display:block; [#else] display:none; [/#if]">
														...
														<div class="col-xs-4">
															<select name="memberrankId" class="selectpicker form-control" data-live-search="true" data-size="10">
																[#list memberRankList as memberRank]
																	<option value="${memberRank.id}" [#if product.memberrankId?? && product.memberrankId==memberRank.id] selected [/#if]>${memberRank.name}</option>
																[/#list]
															</select>
														</div>
													</div>
