...
<head>
				...
				<%_.each(skus, function(entries, i) {%>
					...
					<tr data-ids="<%-ids.join(",")%>">
						<%
                			...
                			var rewardDuiBan = skuValue != null && skuValue.rewardDuiBan != null ? skuValue.rewardDuiBan : "";
                            var rewardYongjin = skuValue != null && skuValue.rewardYongjin != null ? skuValue.rewardYongjin : "";
						...
						<td>
							<input name="skuList[<%-i%>].rewardDuiBan" class="reward-duiban form-control" type="text" value="<%-rewardDuiBan%>" maxlength="9"<%-!isEnabled ? " disabled" : ""%>>
						</td>
                        <td>
                            <input name="skuList[<%-i%>].rewardYongJin" class="reward-yongjin form-control" type="text" value="<%-rewardYongjin%>" maxlength="9"<%-!isEnabled ? " disabled" : ""%>>
                        </td>
						<%} else if (type == "exchange") {%>
							<td>
	...
	<script type="text/javascript">
		$().ready(function() {
            ...
            var $rewardDuiban = $("#rewardDuiban");
            var $rewardYongjin = $("#rewardYongjin");
			...
			function changeView() {
				if (hasSpecification) {
					...
					$price.add($cost).add($marketPrice).add($rewardPoint).add($exchangePoint).add($rewardDuiban).add($rewardYongjin).add($stock).prop("disabled", true).closest("div.form-group").velocity("slideUp");
					switch ($type.val()) {
						case "general":
					...
					switch ($type.val()) {
						case "general":
							$price.add($rewardPoint).add($rewardDuiban).add($rewardYongjin).add($promotionIds).prop("disabled", false).closest("div.form-group").velocity("slideDown");
			...
			function buildSkuTable() {
				...
				$sku.find("tr").each(function() {
					...
					skuValues[$element.data("ids")] = {
                        ...
                        rewardDuiban: $element.find("input.reward-duiban").val(),
                        rewardYongJin: $element.find("input.reward-yongjin").val(),
		...
		function showLimitBuyCount(obj){
		   var val= $(obj).val();
		    if(val == "true"){
		        $("#limitBuyCountDiv").show();
                $("#limitBuyCount").removeAttr("disabled");

			}else{
                $("#limitBuyCountDiv").hide();
                $("#limitBuyCount").prop("disabled","disabled");
			}
		}
        ...
        function validate() {
            ...
            var rewardDuiBan = $("#rewardDuiBan").val() *1;
            var rewardYongJin = $("#rewardYongJin").val() *1;
            if (rewardDuiBan > price || rewardYongJin > price || (rewardDuiBan + rewardYongJin) > price) {
                return "赠送兑伴积分或返佣不能大于商品价格";
            }*/
            return "true";
        }
										...
										<div class="tab-content">
											<div id="base" class="tab-pane active">
												...
												<div class="form-group">
													...
													<div class="col-xs-4">
														<select id="type" name="type" class="selectpicker form-control" data-size="5" [#if gradeGiftBag ] onchange="getProductType(this);" [/#if]>
															[#list types as type]
												...
												<div class="form-group">
													<label class="col-xs-2 control-label item-required" for="caption">商品营销描述:</label>
													<div class="col-xs-4">
														<input id="caption" name="caption" class="form-control" type="text" maxlength="200">
													</div>
												</div>
												<div class="form-group">
													<label class="col-xs-2 control-label item-required" for="price">兑换价格:</label>
													<div class="col-xs-4">
												...
												<div class="form-group">
													...
													<div class="col-xs-4">
                                                        ...
                                                        <label class="control-label" >图片尺寸：225 * 170</label>
													</div>
                                                ...
                                                <div class="form-group">
                                                    <label class="col-xs-2 control-label" for="rewardDuiBan">赠送兑伴儿积分:</label>
                                                    <div class="col-xs-4">
                                                        <input id="rewardDuiBan" name="sku.rewardDuiBan" class="form-control" type="text" maxlength="9" title="赠送兑伴">
                                                        <label class="control-label" >该值为奖励购买者所赠送的积分数额，如该购买用户老用户推荐购买，则会额外赠送给推荐这该值10%数额的兑伴积分作为奖励</label>
                                                    </div>
                                                    
                                                </div>
                                                <div class="form-group">
                                                    <label class="col-xs-2 control-label" for="rewardYongJin">邀请人返佣:</label>
                                                    <div class="col-xs-4">
                                                        <input id="rewardYongJin" name="sku.rewardYongJin" class="form-control" type="text" value="${product.defaultSku.rewardYongJin}" maxlength="9" title="邀请人返佣">
                                                    </div>
                                                </div>
												...
												<div class="form-group">
													...
													<div class="col-xs-10 checkbox">
                                                        ...
                                                        <input id="isLimitBuy" name="isLimitBuy" type="text" value="false" data-toggle="checkbox-x" onchange="showLimitBuyCount(this)">
                                                        <label for="isLimitBuy" class="cbx-label">是否限购</label>
													</div>
												</div>
                                                <div class="form-group" id="limitBuyCountDiv" style="display: none">
                                                    <label class="col-xs-2 control-label" for="limitBuyCount">限购次数:</label>
                                                    <div class="col-xs-4">
                                                        <input id="limitBuyCount" name="limitBuyCount" class="form-control" type="text" maxlength="200" disabled="disabled">
                                                    </div>
                                                </div>
												<div class="form-group">
												...
												[#--<div class="form-group">
													<label class="col-xs-2 control-label" for="keyword">${message("Product.isGradeDistribution")}:</label>
													<div class="col-xs-4">
														<input name="isGradeDistribution" type="radio" value="true" checked>
														<label for="taskShowFlag" class="cbx-label">是</label>
														<input name="isGradeDistribution" type="radio" value="false">
														<label for="taskShowFlag" class="cbx-label">否</label>
													</div>
												</div>--]
												[#if gradeGiftBag]
												<div class="form-group" id="gradeDistributionFlag">
													<label class="col-xs-2 control-label" for="keyword">是否开启等级分配:</label>
													<div class="col-xs-4">
														<input name="gradeDistributionFlag" type="radio" value="true">
														<label for="taskShowFlag" class="cbx-label">是</label>
														<input name="gradeDistributionFlag" type="radio" value="false" checked>
														<label for="taskShowFlag" class="cbx-label">否</label>
													</div>
												</div>
												<div class="form-group" id="memberrankDiv" style="display: none;">
													<label class="col-xs-2 control-label">会员等级:</label>
													<div class="col-xs-4">
														<select name="memberrankId" class="selectpicker form-control" data-live-search="true" data-size="10">
															[#list memberRankList as memberRank]
																<option value="${memberRank.id}">${memberRank.name}</option>
															[/#list]
														</select>
													</div>
												</div>
											...
											<div id="productImage" class="tab-pane">
                                                ...
                                                <label class="control-label" >图片尺寸：500 * 500</label>
											</div>
									...
									<div class="box-footer">
										<div class="row">
											<div class="col-xs-4 col-xs-offset-2">
												<button class="btn btn-primary" id="submitButton" onclick="tijiao()" type="button">${message("business.common.submit")}</button>
...
<script>
$('[name="gradeDistributionFlag"]').click(function () {
	if ($(this).val() == "true") {
		$("#memberrankDiv").attr("style","display:block");
		$('[name="memberrankId"]').removeAttr("disabled");
	}else {
		$("#memberrankDiv").attr("style","display:none");
		$('[name="memberrankId"]').attr("disabled","disabled");
	}
})
function getProductType(obj) {
	let typeValue=$(obj).find("option:selected").attr("value");
	if(typeValue=="general" || typeValue=="coupon"){
		$("#gradeDistributionFlag").attr("style","display:block");
		let gradeDistributionFlag=$('input[name="gradeDistributionFlag"]:checked').val();
		if (gradeDistributionFlag=='true') {
			$("#memberrankDiv").attr("style","display:block");
			$('[name="memberrankId"]').removeAttr("disabled");
		}else {
			$("#memberrankDiv").attr("style","display:none");
		...
		$("#gradeDistributionFlag").attr("style","display:none");
		$('[name="memberrankId"]').attr("disabled","disabled");
		$("#memberrankDiv").attr("style","display:none");
	}
}
